...
dbutils.widgets.text("p_table_list_string","Brand__c")
...
raw_adl_result_path = "abfss://raw@" + storage_account + ".dfs.core.windows.net/CDS_QA/Pitcher/ProfilingComparison" 
...
for table in table_list:
    ...
    raw_adl_act_dir = "CDS_QA/Pitcher/Tables/" + table + "/ADLProfiling/"
    raw_adl_act_path = "abfss://raw@" + storage_account + ".dfs.core.windows.net/" + raw_adl_act_dir 
    
    #define url to expected data set
    raw_adl_exp_dir = "CDS_QA/Pitcher/Tables/" + table + "/SourceProfiling/" + table + "*.csv"
    ...
    if Error_Details != 'Data not found':
        ...
        row_count_exp = df_exp.select("TOTAL_ROW_COUNT").first()["TOTAL_ROW_COUNT"] 
        row_count_act = df_act.select("TOTAL_ROW_COUNT").first()["TOTAL_ROW_COUNT"] 
